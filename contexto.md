...
> Actualizado: 2026-07-06 (Sesión 31e — **LiveTrackingView: click en posición → flyTo preciso**: definición de producto — una posición es un punto tomado por el GPS cada X segundos. Click en un ítem de la lista selecciona la posición (card azul + ring, key estable `item.idx` = índice del trail) y el mapa hace `flyTo` al punto con `minZoom=17` (prop nueva de `FlyToPoint`) mostrando un marcador dot azul con anillo pulsante (`createPositionMarkerIcon`, reutiliza la animación `event-ring-expand`). Click de nuevo deselecciona; al cerrar el panel se limpia (`selectedPosIdx`). El copy de coords ahora usa `stopPropagation` para no disparar la selección.)
> Actualizado: 2026-07-06 (Sesión 31d — **LiveTrackingView: header responsivo con container query**: la card de info de la unidad ahora es `@container` (Tailwind 4). Cuando la card mide ≥640px la ubicación (dirección + coords, extraída al const `locationBlock`) va inline entre la identidad y los botones, con divisores verticales; cuando es más angosta (p.ej. con el panel de Posiciones/Eventos abierto) la ubicación pasa a una segunda fila debajo, separada con `border-t`, y los botones se alinean a la derecha con `ml-auto`. Esto elimina el wrap vertical palabra por palabra de la dirección.)
> Actualizado: 2026-07-06 (Sesión 31c — **Tooltip "Copiado ✓" en lista de posiciones**: al copiar coordenadas de un ítem, el feedback ya no es un chip inline junto a las coords sino el tooltip flotante estándar (`absolute -top-7 left-0 bg-slate-800 text-white px-2 py-1 rounded-md shadow-lg z-50`, dark: `bg-zinc-700 text-zinc-200`) — mismo patrón que las coords de VehicleAccordionItem. Además, la card del ítem ya no es clickeable: el copy solo se dispara al hacer click sobre las coordenadas, que ahora son un botón propio con hover estándar del buscador (`hover:text-brand hover:underline` / dark `hover:text-blue-400` + ícono Copy visible al hover, patrón GpsPopover). Aplicado en LiveTrackingView y VehicleCaptureView.)
> Actualizado: 2026-07-06 (Sesión 31b — **LiveTrackingView: botón Eventos**: segundo toggle junto a "Posiciones" (mismo estilo pill Eye/EyeOff). Los paneles son excluyentes (estado `activePanel: 'none' | 'positions' | 'events'`). El panel "EVENTOS DEL RECORRIDO n" replica el estándar de "Eventos del viaje" de VehicleTripView: filtros Todos / Filtrar (dropdown multi-selección con Checkbox y contador por tipo), lista de eventos (dirección + hora + ícono/métrica por tipo), botón X de cierre y scroll hint. Los eventos son mock generados sobre puntos del trail inicial (5–8 según seed de `vehicle.id`, tipos de EVENT_CONFIG). En el mapa: marcadores de evento (`createEventMarkerIcon` e `injectFlowCss` ahora exportados desde VehicleTrackingMap) sobre el recorrido punteado — visible con cualquiera de los dos paneles activos; click en evento (lista o mapa) selecciona + `flyTo` (componente `FlyToPoint`); los tipos filtrados se atenúan en el mapa (opacity 0.28).)
> Actualizado: 2026-07-06 (Sesión 31 — **LiveTrackingView: botón Posiciones**: se eliminaron las métricas del header (Odómetro, Velocidad, Combustible, Alertas) y en su lugar hay un botón toggle "Posiciones" (Eye/EyeOff, pill azul cuando activo). Al activarlo: (1) panel de posiciones `w-[272px]` entre el panel de cámaras y el mapa — misma card que VehicleCaptureView (header "POSICIONES n" + RefreshCw, lista con "Última" resaltada, copy de coords, scroll hint animado); (2) el mapa dibuja el recorrido como polyline punteada azul + marcador "A" de origen (`createRouteMarkerIcon` ahora exportado desde VehicleTrackingMap) y hace fitBounds al recorrido; el seguimiento en vivo (`VehicleFollower`, nueva prop `active`) se pausa mientras el panel está abierto. El recorrido es un trail sintético de 20 puntos hacia atrás desde la posición actual (seed por `vehicle.id`, intervalo 30 s) al que se agregan puntos en vivo cuando el vehículo se mueve vía VehicleContext.)
